import { requireApiSession, isSameOrigin } from '@/lib/auth/session';
import { deleteFaqItem, updateFaqItem, FAQ_PAGES } from '@/lib/store/faq';
import { validateFaqItem } from '@/lib/validation';

/** Egy gyakori kérdés módosítása és törlése. */
export const runtime = 'nodejs';

const PAGE_KEYS = FAQ_PAGES.map((page) => page.key);

export async function PUT(
  request: Request,
  { params }: { params: Promise<{ id: string }> },
): Promise<Response> {
  if (!isSameOrigin(request)) {
    return Response.json({ ok: false, error: 'Érvénytelen kérés.' }, { status: 403 });
  }

  const session = await requireApiSession();
  if (!session.ok) return session.response;

  let payload: unknown;
  try {
    payload = await request.json();
  } catch {
    return Response.json({ ok: false, error: 'Hibás kérés.' }, { status: 400 });
  }

  const result = validateFaqItem(payload, PAGE_KEYS);
  if (!result.ok) {
    return Response.json(
      { ok: false, errors: result.errors, error: 'Néhány mezőt javítani kell.' },
      { status: 422 },
    );
  }

  const { id } = await params;
  const item = await updateFaqItem(id, result.value);
  if (!item) {
    return Response.json({ ok: false, error: 'Nincs ilyen kérdés.' }, { status: 404 });
  }

  return Response.json({ ok: true, item });
}

export async function DELETE(
  request: Request,
  { params }: { params: Promise<{ id: string }> },
): Promise<Response> {
  if (!isSameOrigin(request)) {
    return Response.json({ ok: false, error: 'Érvénytelen kérés.' }, { status: 403 });
  }

  const session = await requireApiSession();
  if (!session.ok) return session.response;

  const { id } = await params;
  const removed = await deleteFaqItem(id);
  if (!removed) {
    return Response.json({ ok: false, error: 'Nincs ilyen kérdés.' }, { status: 404 });
  }

  return Response.json({ ok: true });
}
