...
import { deleteTeamMember, updateTeamMember } from '@/lib/store/team';
import { validateTeamMember } from '@/lib/validation';

/** Egy csapattag módosítása és törlése. */
...
export async function PUT(
  ...
  const member = await updateTeamMember(id, result.value);
  if (!member) {
    return Response.json({ ok: false, error: 'Nincs ilyen csapattag.' }, { status: 404 });
  }
