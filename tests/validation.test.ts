...
import { slugify, validateContact, validatePost, validatePriceValue } from '@/lib/validation';
...
describe('validatePriceValue', () => {
  it('nem enged üres árat', () => {
    expect(validatePriceValue('   ').ok).toBe(false);
  });

  it('levágja a felesleges szóközöket', () => {
    const result = validatePriceValue('  100 000 Ft-tól ');
    expect(result.ok).toBe(true);
    if (result.ok) expect(result.value).toBe('100 000 Ft-tól');
  });
});
